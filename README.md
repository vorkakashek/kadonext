...
`/var/www/kadonext/releases/<timestamp>-<commit>`. После проверки файлов
скрипт устанавливает `ops/nginx/kadonext.conf`, проверяет `nginx -t`, атомарно
переключает симлинк `/var/www/kadonext/current`, перезагружает nginx и проверяет
русскую и английскую версии `/`. При ошибке активации прежние конфигурация и
релиз восстанавливаются. Nuxt-процесс на сервере не нужен: сайт статический.
...
`/ru/projects/`, загрузку шрифта и выбор языка на `/` без перенаправления.
