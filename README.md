...
To test recording offline on the development computer, run `npm run dev` and open `http://localhost:3000` (or the port printed by Nuxt) in a browser with microphone permission. Recording, playback and saving messages on the page work without an internet connection; sending email requires a working gateway and SMTP connection. A plain HTTP LAN IP is not a secure context. Offline phone testing requires local HTTPS with a certificate trusted by the phone; an HTTPS tunnel is an online alternative. Record → stop → play → save, then switch to text and back to check that the saved message and text remain available. Unsaved recordings are only kept in the current browser session; reloading the page clears them.
